        ...
        <div className="articles-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {ARTICLES.map(article => (
            <TiltCard key={article.id} className="h-full">
              <Link to={`/article/${article.id}`} className="article-card bg-white dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800 p-10 rounded-[2.5rem] shadow-sm hover:shadow-2xl transition-all flex flex-col h-full group relative overflow-hidden block outline-none focus-visible:ring-4 focus-visible:ring-blue-500">
                ...
                <div className="relative z-10 flex flex-col h-full pointer-events-none">
                  ...
                  <div>
                    <span className="inline-block bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-white px-8 py-4 rounded-2xl font-black text-center border border-slate-200 dark:border-slate-700 group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 transition-all uppercase tracking-widest text-xs">Изучить модуль</span>
                  </div>
                </div>
              </Link>
            </TiltCard>
