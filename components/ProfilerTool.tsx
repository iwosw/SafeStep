...
interface ProfilerToolProps {
  onComplete?: () => void;
}

export const ProfilerTool: React.FC<ProfilerToolProps> = ({ onComplete }) => {
  ...
  const runAnalysis = () => {
    ...
    setTimeout(() => {
      ...
      setProfile({
        ...
        insight: isNightOwl ? "Склонность к импульсивным покупкам в ночное время." : "Стабильный пользователь с предсказуемым поведением.",
        avatar: isTech 
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto p-4 relative">
      <div className="absolute inset-0 opacity-[0.02] pointer-events-none">
        <img 
          src="https://images.unsplash.com/photo-1558494949-ef010cbdcc51?w=1200&h=1200&fit=crop" 
          className="w-full h-full object-cover" 
          alt="Profiler Background" 
          referrerPolicy="no-referrer"
        />
      </div>
      <div className="relative z-10">
        <div className="text-center">
          <h3 className="text-3xl font-black text-[#1e3c72]">Digital Behavioral Profiler 🧬</h3>
          <p className="text-gray-500 mt-2">Выберите 2-3 действия, чтобы увидеть, какой профиль составит о вас алгоритм.</p>
        </div>
        ...
        <div className="text-center my-8">
          ...
          >
            {analyzing ? 'АНАЛИЗИРУЮ ДАННЫХ...' : 'ЗАПУСТИТЬ ПРОФАЙЛЕР 🚀'}
          </button>
        </div>
        ...
        <div className="bg-[#0f111a] rounded-[3rem] p-1 md:p-10 shadow-2xl border-b-8 border-[#1a1c23] relative overflow-hidden min-h-[350px]">
          ...
          {analyzing ? (
              ...
              <div className="flex flex-col md:flex-row gap-10">
                <div className="flex-grow space-y-6">
                  <div className="border-b border-white/10 pb-4 flex items-center gap-4">
                      <img 
                        src="https://images.unsplash.com/photo-1557591954-83086ad8914d?w=100&h=100&fit=crop" 
                        className="w-12 h-12 rounded-lg opacity-50 grayscale hover:grayscale-0 transition-all" 
                        alt="ID Scan" 
                        referrerPolicy="no-referrer"
                      />
                      <div>
                  ...
                  <div className="bg-blue-600/10 border border-blue-500/30 p-6 rounded-[2rem]">
                      <div className="text-[10px] text-blue-400 font-black uppercase mb-2">Digital Behavioral Insight</div>
                      <p className="text-gray-300 text-sm italic">"{profile.insight}"</p>
                  </div>
                </div>

                <div className="md:w-1/3 bg-white/5 border border-white/10 p-6 rounded-[2rem] flex flex-col items-center text-center">
                   <div className="relative mb-6">
                      <div className="absolute inset-0 bg-blue-500/20 blur-2xl rounded-full"></div>
                      <img 
                        src={profile.avatar} 
                        className="w-32 h-32 object-cover rounded-full border-4 border-blue-500/30 relative z-10" 
                        alt="Generated Persona" 
                        referrerPolicy="no-referrer"
                      />
                   </div>
                   <div className="text-white text-xs font-mono mb-4 uppercase">Behavioral Map</div>
            ...
            <div className="flex flex-col items-center justify-center h-full py-20 text-center text-gray-500 relative z-10">
              <div className="relative mb-8">
                ...
                <img 
                  ...
                  className="w-32 h-32 object-cover rounded-3xl border-2 border-white/10 opacity-40 grayscale group-hover:grayscale-0 transition-all duration-700" 
                  alt="Neural Network Placeholder" 
                  referrerPolicy="no-referrer"
                />
              </div>
              <p className="max-w-xs mx-auto text-sm italic font-medium text-gray-400">Алгоритмы анализируют не только ЧТО вы делаете, но и КОГДА и КАК. Ваша цифровая "тень" рассказывает о вас всё.</p>
            </div>
          )}
        </div>
      </div>
